...
// common
constexpr std::uint32_t type_size = sizeof(std::uint8_t);
constexpr std::uint32_t cell_count_size = sizeof(std::uint32_t);

constexpr std::uint32_t key_size = sizeof(std::uint32_t);
